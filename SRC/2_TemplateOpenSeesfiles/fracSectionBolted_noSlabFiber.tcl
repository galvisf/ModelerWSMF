...
# slabMatCompTag	Material ID for the concrete in the slab
# slabMatTenTag 	Material ID for the steel in the slab
...
#
proc fracSectionBolted {secTag eleDir topFMatTag botFMatTag webMatTag d bf tf ttab boltLocation boltDiameter FuvBolt FyTab FuTab Lc tabLength du_div Composite slabMatCompTag slabMatTenTag trib tslab bslab AslabSteel} {
	...
	if {$Composite} {
		...
		set Aslab [expr ($tslab + $trib/2) * $bslab]
	}
	...
	section Fiber $secTag {	
		...
		if {$Composite} {
			if {$eleDir == "Horizontal"} {	
				fiber $yslab $z $Aslab $slabMatCompTag;
				fiber $yslab $z $AslabSteel $slabMatTenTag;
			}
		}		
	}
}
