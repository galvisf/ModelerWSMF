...
#
proc hingeBeamColumnSplice { eleTag node_i node_j eleDir transfTag n Es Fy rigMatTag A Ieff degradation c secInfo_i secInfo_j spliceLoc  spliceSecGeometry FySplice sigCr ttab dtab} {
	...
	if {$eleDir == "Horizontal"} {
		...
		set y3 $y1;
		node $nodeSpl1 [expr $x3 - 2] $y3;	
		node $nodeSpl2 [expr $x3 + 2] $y3;
	} else {
		set x3 $x1
		set y3 [expr $y1 + $spliceLoc];
		node $nodeSpl1 $x3 [expr $y3 - 2];	
		node $nodeSpl2 $x3 [expr $y3 + 2];
	}	
	...
	## Define splice materials ##
	# puts "FySplice=$FySplice"
	# puts "sigCr=$sigCr"
	set webMatTag [expr $hingeSecTag_i+10]
	set fracMatTag [expr $hingeSecTag_j+20]	
	matSplice $fracMatTag $sigCr $Es $FySplice
	...
	# WsectionSplice $spliceSecTag $fracMatTag $webMatTag $d $bf $tf $tw $nfdw $nftw $nfbf $nftf;
	set FyWeb [expr min($sigCr, $FySplice)]
	fracSectionWelded $spliceSecTag $eleDir $fracMatTag $fracMatTag $webMatTag $d $bf $tf $ttab $dtab $FyWeb $FyWeb $Es 0 0 0 0 0
	...
	# Option 2: DO NOT USE BECAUSE STIFFNESS MODIFIERS ARE NOT CORRECT WHEN USING MORE THAN ONE ELASTIC ELEMENT FOR THE MEMBER
	# set K44_2 [expr 6*(1+$n)/(2+3*$n)];
	# set K11_2 [expr (1+2*$n)*$K44_2/(1+$n)];
	# set K33_2 [expr (1+2*$n)*$K44_2/(1+$n)];
	# set IeffElement [expr $Ieff * ($n+1) / $n]
	# element ModElasticBeam2d $eleTag $nodeInt1 $nodeSpl1 $A $Es $IeffElement $K11_2 $K33_2 $K44_2 $transfTag
	# element ModElasticBeam2d $eleTag2 $nodeSpl2 $nodeInt2 $A $Es $IeffElement $K11_2 $K33_2 $K44_2 $transfTag

	# Splice element
	set numIntgrPts 5
	set elemConvTol 1e-4
	element forceBeamColumn $spliceEleTag $nodeSpl1 $nodeSpl2 $numIntgrPts $spliceSecTag2 $transfTag -iter 50 $elemConvTol
