...
proc hingeBeamColumnFracture { eleTag node1 node2 eleDir transfTag n Es Fy rigMatTag A Ieff degradation c secInfo_i secInfo_j webConnection fracSecGeometry fracSecMaterials sigCrB_i sigCrT_i sigCrB_j sigCrT_j FI_limB_i FI_limT_i FI_limB_j FI_limT_j Composite compBackboneFactors trib tslab bslab AslabSteel slabFiberMaterials} {
	...
	uniaxialMaterial SteelFractureDI $botFMatTag_i $FyFiber $FyFiber $EsFiber 0.02 18 0.85 0.15 0.08 1.00 0.08 1.00 $sigCrB_i $betaC_B $sigMin $FI_limB_i;
	uniaxialMaterial SteelFractureDI $topFMatTag_i $FyFiber $FyFiber $EsFiber 0.02 18 0.85 0.15 0.08 1.00 0.08 1.00 [expr 2*$sigCrT_i] $betaC_T $sigMin $FI_limT_i;
	uniaxialMaterial SteelFractureDI $botFMatTag_j $FyFiber $FyFiber $EsFiber 0.02 18 0.85 0.15 0.08 1.00 0.08 1.00 $sigCrB_j $betaC_B $sigMin $FI_limB_j;
	uniaxialMaterial SteelFractureDI $topFMatTag_j $FyFiber $FyFiber $EsFiber 0.02 18 0.85 0.15 0.08 1.00 0.08 1.00 [expr 2*$sigCrT_j] $betaC_T $sigMin $FI_limT_j;
	...
	set compositeFiber 0; # DO NOT ADD SLAB FIBERS
	if {$webConnection == "Bolted"} {	
		# Bolted web connection		
		#set du_div 2; #Fraction of nominal displacement capacity of web fiber material
		fracSectionBolted $fracSecTag_i $eleDir $topFMatTag_i $botFMatTag_i $webMatTag_i $d $bf $tf $ttab $boltLocation $boltDiameter $FuBolt $FyTab $FuTab $Lc $tabLength $du_div $compositeFiber $slabMatTag $trib $tslab $bslab $AslabSteel;
		fracSectionBolted $fracSecTag_j $eleDir $topFMatTag_j $botFMatTag_j $webMatTag_j $d $bf $tf $ttab $boltLocation $boltDiameter $FuBolt $FyTab $FuTab $Lc $tabLength $du_div $compositeFiber $slabMatTag $trib $tslab $bslab $AslabSteel;
		
	} else {
		# Fully welded web connection		
		fracSectionWelded $fracSecTag_i $eleDir $topFMatTag_i $botFMatTag_i $webMatTag_i $d $bf $tf $ttab $dtab $FyTab $FuTab $EsFiber $compositeFiber $slabMatTag $trib $tslab $AslabSteel;
		fracSectionWelded $fracSecTag_j $eleDir $topFMatTag_j $botFMatTag_j $webMatTag_j $d $bf $tf $ttab $dtab $FyTab $FuTab $EsFiber $compositeFiber $slabMatTag $trib $tslab $AslabSteel;
	...
	if {$eleDir == "Horizontal"} {
		...
		element zeroLengthSection $fracEleTag_j $nodeInt4 $node2 $fracSecTag2_j -orient 1 0 0 0 1 0 -doRayleigh 1; # local x is global X and local y is global Y			
	} else {
		# Option 1: Constraint
		# equalDOF $nodeInt1 $nodeInt2 1 2
		# equalDOF $nodeInt3 $nodeInt4 1 2
		...
		element zeroLengthSection $fracEleTag_j $nodeInt4 $node2 $fracSecTag2_j -orient 0 1 0 -1 0 0 -doRayleigh 1; # local x is global Y and local y is global -X		
	}
}
