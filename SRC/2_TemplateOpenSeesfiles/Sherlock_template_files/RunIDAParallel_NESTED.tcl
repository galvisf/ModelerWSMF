#############################################################################################################
# Kuanshi Zhong (with adaptations from Reagan)                                                              #
...
set inpath ./GroundMotion
set indirlist {N49_1D_7x7_T4.3s}

# Define the output path
set outpath AnalysisResult/IDA

# Define the collapse peak story drift ratio
set col_drift 0.10
set elemConvTolMax 1e-6;
...
if {$this_procid == 0} {
    ...
    foreach indir $indirlist {
        ...
        set sat1file [open $inpath/$indir/SaT1_[format "%.2f" $T1]s.out r]
    ...
    set delta_sat1_coarse 0.1
    set numforks_coarse 5
        ...
        dict with gminfo {
            ...
            if {[file exists $outpath/$indir/$outfilename/completed_scales.txt]} {
                ...
                if {[dict get $numscales 2] == $numforks_fine} {
					...
					# changed here, write completed GM to file
                    set completed_gm_file [open $outpath/$indir/completed_gm.txt a];
                    puts $completed_gm_file "[format "%d" [expr {$gm + 1}]]"; # changed here
                    close $completed_gm_file
    ...
    # and terminate execution. Else, loop until all ground motions have been scaled to cause collapse and the
    # "runlist" stack is empty.
    ...
    while {$sum_collapsed < $numgms || [llength $runlist]} {
        ...
        # Assign jobs to all free processors by popping ground motions and scales from the "runlist" stack and
        # free processors from the "free_procs" stack
        ...
        # Receive the results of a completed job and push the id of the processor that finished its job to the
        # "free_procs" stack
        recv -pid ANY result
		...
		set tLast [lindex $result 5]; # changed here
        set max_drift_last [lindex $result 6]; # changed here
        set state [lindex $result 7]; # changed here

        # Modify the data structures depending on whether the job was from the coarse or fine stage and the 
        # results of the job
        switch $tier {
            1 {
                ...
                # If the analysis did not cause collapse, add the entry to the "ida_curves" dictionary, else
                # update the "lowest_fork_collapse" entry for the ground motion if necessary
                if {$drift < $col_drift} {
                    dict set ida_curves $gm $sat1 $drift
                } else {
                    set lowest_fork_collapse($gm) [expr {min($lowest_fork_collapse($gm), $sat1)}]
                ...
                # Write the result to the "completed_scales.txt" file in the output folder corresponding to
                # the current ground motion
                set gminfo [dict get $gminfo_dict $gm]
                dict with gminfo {
                    ...
                    puts $completed_scales_file "[format "%.3f" $sat1]\t[format "%.5f" $drift]\t$tier\t[format "%.5f" $tLast]\t[format "%.5f" $max_drift_last]\t[format "%s" $state]"; # changed here
                    close $completed_scales_file
                }

                # If all coarse scale forks have finished running and none of the scales have caused collapse,
                # start a new coarse stage with higher scales, else, reset the "forks_run" counter and push
                # the ground motion with scales corresponding to the fine stage into the "runlist" stack.
                # These fine scales are computed based on the "lowest_fork_collapse" entry for the ground
                # motion.
                if {[expr {$forks_run($gm) % $numforks_coarse}] == 0} {
                    if {$lowest_fork_collapse($gm) == inf} {
                        ...
                        set forks_run($gm) 0
                        for {set i 1} {$i <= $numforks_fine} {incr i} {
                            lappend runlist "$gm [expr {$lowest_fork_collapse($gm) - $i*$delta_sat1_fine}] 2"
                        }
                    }
                }
            }
            ...
            2 {
                ...
                # If all fine scale forks have finished running, update the flag corresponding to the ground
                # motion in the "collapsed" list to indicate that the ground motion has been scaled to cause
                # collapse and display a message
...
# Tasks to be executed by all slave processors
    ...
    for {set i 0} {$status != "DONE"} {incr i} {
        ...
        dict with gminfo {   
			...
			set outdir $outpath/$indir/$outfilename/Scale_[format "%.2f" $sat1]
			file mkdir $outdir
			
			# Specify NAME OF THE MODEL
			source DriftCheck.tcl
			source ModelBuilt.tcl
			
			# Perform analysis
			source RecorderAnalysisIDA.tcl;
        }

        # Send the master node the results of the analysis and receive a new message
        send -pid 0 "$this_procid $gm $sat1 $tier $max_drift $tLast $max_drift_last $state"; # changed here
        recv -pid 0 command
        set status [lindex $command 0]
    }
}
